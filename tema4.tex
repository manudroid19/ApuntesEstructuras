...
\section{Módulos de tipo finito sobre un DIP}
	...
	\begin{theorem}
		...
		tal que $ (0:Z_1) contains al reves (0:Z_2) contains reves ... contains reves (0:Z_s), \quad (0:Z_n\neq D) $
		
	\end{theorem}
	\begin{proof}
		Sea $ \{X_1,\cdots, X_n\} $ un conjunto finito de generadores de M. Sea $ F $ un $ D $-módulo libre de rango $ n $ con base $ \{e_1, \cdots e_n\} $, y sea $ \epsilon:F\flechaSobreyectiva M $ el homomorfismo sobreyectivo $ \epsilon(e_i)=x_i, 1\leq i\leq n $.
		
		Sea $ N:=\text{Ker}\epsilon $:
		\[ N\flechaInyectiva F \xFlechaSobreyectiva{\epsilon} M \]
		...
		\vdots \\
		e_m
		\end{bmatrix} PONER PARENT NO CONRCH\]
		lo cual no es un producto de matrices en el sentido usual, ya que $ f,e $ no son matrices sobre $ D $, pero la notación será igualmente útil. Se comprueba facilmente la asociatividad usual.
		\[ B(cf)=(Bc)f \]
		...
		([Importante en aplicaiones, no en demostración]Nótese que como los $ e'_i $ son linealmente independientes y los $ d_i $ son no nulos, resulta que $ f'_1, \cdots, f'_r $ son linealmente independientes. Por tanto, $ N $ es libre de rango $ r $ y $ \{ f'_1, \cdots, f'_r\} $ es base de $ N $. Esto proporciona un método para encontrar una base de un submódulo de un $ D $-módulo libre a partir de un conjunto de generadores.)
		Sea $ Y_i := \epsilon(e'_i), 1 \leq i \leq n$. (Nótese que $ Y=P^{-1}X $).\\
		$ e'=P^{-1}e, Y=P^{-1}X $\\
		Como $ \epsilon $ es sobre, resulta que $ \{y_1, \cdots y_n \} $ es un conjunto de generadores de $ M $. 
		\[ M=\sum_{j=1}^{n} DY_i\]
		Veamos que esta es directa, i.e.,
		\[ \sum_{j=1}^{n} b_i\cdot y_i=0, \quad b_i \in D \Rightarrow b_i\cdot y_i=0, \forall i \]
		Se tiene: 
		\[ 0=\sum_{i=1}^{n} b_i \cdot y_i = \sum_{i=1}^{n} b_i \cdot \epsilon(e'_i)=\epsilon (\sum_{i=1}^{n} b_i \cdot e'_i)\Rightarrow \]
		...
		b_i=0, \quad r+1 \leq i \leq n \Rightarrow b_i y_i =0\\
		b_i=c_i\cdot d_i, \quad 1 \leq i \leq r \Rightarrow ¿?
		FALTAAAA
		\end{cases}\]
		Así pues, 
		...
		"$ \supset $" Ya está demostrado.\\
		"$ \subset $" $ \lambda \in (0:y_i) $
		FALTAAAA
		Sea $ s $, $ 0 \leq s \leq n $, tal que $ (0:y_1)=(0:y_2)=\cdots =(0:y_n-s)=D $, $ (0:y_{n-s+1})\neq D $. Entonces: $ y_1=y_2=\cdots = y_{n-s}=0 $\\
		$ Z_1:=Y_{n-s+1},Z_2:=Y_{n-s+2}, \cdots, Z_s:=Y_n  $.\\
		Se tiene: 
		\[ M=DZ_1\oplus \cdots \oplus DZ_s \]
		y $ (0:z_1)\supset(0:z_2)\supset\cdots =(0:z_s)$, con $ (0:Z_k)\neq D $.
	\end{proof}
	\begin{note}
		Sea $ M $ un $ D $-módulo
	\end{note}
