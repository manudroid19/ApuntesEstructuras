...
\newcommand{\docabstract}{Tema impartido por Javier Majadas, curso 2017/2018.}
...
\section{Módulos}
	...
	\begin{definition}
        	Un $A$-módulo \index{$A$-módulo} es un grupo abeliano $M$ con una operación:
	...
	\begin{definition}
		...
		Se dice que $N$ es un $A$-submódulo\index{$A$-submódulo} de $M$ si $N$ es un $A$-módulo con la operación de $A$ en $M$.
	\end{definition}
	...
	\begin{definition}
			Sea $ \{M_i\}_{i\in I} $ familia de submódulos de $ M $. Definimos en general la suma de submódulos\index{suma de submódulos}:
			\begin{align*}
	...
	\begin{proposition}
		...
		Se dice que $X$ es un conjunto de generadores\index{conjunto de generadores} de $\langle X\rangle$.
	...
	\begin{definition}
		Sean $M$, $N$ $A$-módulo. Una aplicación $f:M \rightarrow N$ es un homomorfismo de $A$-módulos\index{homomorfismo de $A$-módulos} si:
		\begin{enumerate}
		\item  $f(x+y) = f(x)+f(y)$
		\item $f(ax) = af(x)$
		\end{enumerate}
	...
	\begin{definition}
		Si $f: M\rightarrow N$ es un homomorfismo de $A$-módulos se llama conúcleo de $f$\index{conúcleo de $f$} al
		$A$-módulo:
		\[coker(f) = M/Im(f).\]
	\end{definition}

	\begin{definition}
		Un homomorfismo $f:M\rightarrow N$ de $A$-módulos es un isomorfismo \index{isomorfismo de $A$-módulos} si existe un homomorfismo de $A$-módulos $g:N\rightarrow M / g\circ f = id_M$, $f\circ g = id_N$. En este caso es $g$ único y $g=f^{-1}$.$\\$
		Así, si existe un isomorfismo de $M$ a $N$ se dice que $M$ y $N$ son isomorfos\index{módulos isomorfos} y se denota como $M\simeq N$.
	\end{definition}
	...
	\begin{definition}
		Sea $A$ un anillo. Un $A$-módulo $M$ es cíclico\index{$A$-módulo cíclico} si está generado por un elemento, es decir:
	...
	\begin{definition}
		Sean $A$ anillo y $M$ $A$-módulo. Se llama aniquilador de $M$\index{aniquilador de un $A$-módulo} al ideal de $A$ definido por:
		\[(0:M):=\{a\in A \ / \ ax=0 \  \forall x \in M\}\]
	\end{definition}

	\begin{definition}
		Tomando $M=\langle x\rangle$ definimos el aniquilador de un elemento\index{aniquilador de un elemento}:
	...
	\begin{definition}
		Sea $\{M_i\}_{i\in I}$ una familia de $A$-módulos. Definimos el producto directo de\index{producto directo de $A$-módulos} los $M_i$ como el $A$-módulo:
	...
	\begin{definition}
		Sea $\{M_i\}_{i\in I}$ una familia de $A$-módulos. Definimos la suma directa\index{suma directa de $A$-módulos} de los $M_i$ como el $A$-submódulo de $\prod_{i\in I}M_i$:
	...
	\begin{definition}
		$ A $ dominio. Sea $ M$ un $ A $-módulo. Un elemento $ x\in M $ es de torsión\index{elemento de torsión} si $ (0:x)\neq 0 $.
		(si existe $ a\in A, a \neq 0 $ tal que $ ax=0 $).
		El conjunto de elementos de torsión\index{conjunto de elementos de torsión} de $ M $ se denota $ T(M) $ y es un $ A $-submódulo de $ M $.
	\end{definition}
	...
	\begin{definition}
		Si $ T(M)=M $ se dice que $ M $ es un $ A $-módulo de torsión\index{$ A $-módulo de torsión}. Si $ T(M)=0 $ se dice que $ M $ es un $ A $-módulo libre de torsión\index{$ A $-módulo libre de torsión}.
	\end{definition}
	...
	\begin{definition}
		Sea $ M $ un $ A $-módulo.\\
		Una familia $ \{x_i\}_{i\in I} $ de elementos de $ M $ es linealmente independiente\index{familia $ \{x_i\}_{i\in I} $ de elementos de un $A$-módulo linealmente independiente} si:
	...
	\begin{definition} Una base de un $ A $-módulo\index{base de un $ A $-módulo} $ M $ es un conjunto de generadores linealmente independiente.  \[ M=\langle \{x_i\}_{i\in I}\rangle=\sum_{i\in I} Ax_i\ \  \{x_i\}_{i\in I} \text{ linealmente independiente.} \] 
	\end{definition}
		
	\begin{definition}
		Un $ A $-módulo $ F $ es libre\index{$ A $-módulo libre} si tiene una base $\{x_i\}_{i\in I}$. La base cumple:
		\begin{itemize}
	...
	\begin{definition}
		Sea $M$ un $A$-módulo. Un homomorfismo sobreyectivo de $A$-módulos $\epsilon : F \longrightarrow M$, con $F$ un $A$-módulo libre, se denomina presentación libre\index{presentación libre de un $A$-módulo} de $M$.
	\end{definition}
	\begin{proposition}
		Todo módulo $M$ tiene una presentación libre.
	\end{proposition}
	\begin{definition}
		Sea $\{x_i\}_{i\in I}$ un conjunto de generadores de $M$ y $\epsilon : F = A^{(I)} \longrightarrow M$ una presentación libre de $M$ tal que $\epsilon (e_i) = x_i$. Se denomina módulo de relaciones de esta presentación\index{módulo de relaciones de una presentación libre} a $N = ker(\epsilon) \subset A^{(I)}$.
	\end{definition}
	\begin{proposition}
	...
	\begin{definition}
		El cardinal de una base de un $A$-módulo libre se llama rango del módulo libre\index{rango de un $A$-módulo libre}.
	\end{definition}
	\printindex
\end{document}
