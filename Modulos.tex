...
La interseccion de una dfamilia de submodulos de m es un submoduklos de M.

Sea M un A-modulo, X contenido en m, se llama A-submodulo de m generado por X al menor A-submodulo de M que  contiene a X. Se denota Xpico ao Xpico_subA. 
Se dice que X es un cjto de generadores de Xpico

xpico = llame sum_DEsde 1 hasta n de asub xsubi/npertNaturales, asubipertA, xsubipertX llave

X = llave x llave 

llave x llave pico = xpico = Ax = llave ax/a pert A llave

Xpico = sum xpertX Asubx

sum i Mi = pico Union i pico

Sean M, N A-mod Una aplicacion f:M --------- N es un hom de A-mod si 

i) f(x+y) f(x)+f(y)
ii)f(ax) = af(x)

id sub M : M ------------ M es un homo de modulos
La composicion de hom de A-modulos es un hom de A-modul

f: M -------------- N f(x) = 0 paratodox pert M es un hom de A-modul

f(0) = 0, f(-x) = -f(x)

Si f: M --------- N es un hom de A-modulk

M´ cont M un submodulo /implica f(M´) es un submodulo de N
N´ cont N un submodulo /implica f-1(N´) es un submodulo de M

ker(f) = f-1 (0) es un submodulo de M

im(f) = f(M) """"""" de N

f iny ssi kerf = 0

Sea M un A-modulo, N A-submodulo de M.

M/N x sombrero = x + N (x + N) + (y + N) =x +y + N

a(x + N) = ax + N 
x + N = x´+N tqd ax +N=ax´+N
x-x´pert N /implica a(x +x´) pert N

/implica ax+ax´pert N

pi : M -------------M/n (sigo en libreta prada )
